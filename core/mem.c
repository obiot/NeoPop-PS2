...
_u8 ram[1 + RAM_END - RAM_START] __attribute__((aligned(64)));
...
void* translate_address_read(_u32 address)
{
	...
	if (rom.data && address >= ROM_START && address <= ROM_END)
	{
		if (address <= ROM_START + rom.length)
			return (&rom.data[0] + (address - ROM_START));
		else
			return NULL;
	}
...
_u16 inline loadW(_u32 address)
{  
...
_u32 inline loadL(_u32 address)
{ 
    _u8* ptr = translate_address_read(address);
...
void inline storeB(_u32 address, _u8 data)
{
...
void inline storeW(_u32 address, _u16 data)
{
...
void inline storeL(_u32 address, _u32 data)
{
