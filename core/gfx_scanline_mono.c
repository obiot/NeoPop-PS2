...
static void Plot(_u8 x, _u8* palette_ptr, _u16 pal_hi, _u8 index, _u8 depth)
{
	...
	{
	  if (negative)
	   cfb_scanline[x] = (u16)mono_lookup[(palette_ptr[4 + index] & 0x7)];
	  else
	   cfb_scanline[x] = (u16)mono_lookup[(_u8)(~palette_ptr[4 + index] & 0x7)];
	}
	else
	{
  	  if (negative)
	   cfb_scanline[x] = (u16)mono_lookup[(palette_ptr[0 + index] & 0x7)];
	  else
	   cfb_scanline[x] = (u16)mono_lookup[(_u8)(~palette_ptr[0 + index] & 0x7)];
	}
...
void gfx_draw_scanline_mono(void)
{
	...
	{
		data16 = (u16)(r | g | b);
		rgb16 = (u16)mono_lookup[oowc];
	}
	else
	{
		data16 = (u16)~(r | g | b);
		rgb16 = (u16)mono_lookup[(_u16)~oowc];
	}
	...
	if (scanline >= winy && scanline < winy + winh)
	{
		...
		if ((bgc & 0xC0) == 0x80)
		{
			...
			b = (_u16)(bgc & 7) << 9;
			data16 = ~(r | g | b);
			rgb16 = (u16)mono_lookup[(~bgc & 7)];
		}
		else 
		{
		   data16 = 0x0FFF; //RGB444
		   rgb16 =  0x739C; //RGB555 for display
		}
