...
void DMA_update(int channel)
{
	...
	if(	dmaC[channel]>=4 && size<2 )
	{
		step=4-(size<<1);
		size=2;
	} 
	else if(	dmaC[channel]>=2 && size==0 )
	{
		step=2;
		size=1;
	}
...
void inline dmaStoreB(_u8 cr, _u8 data)
{
#ifdef NEOPOP_DEBUG
	if (filter_dma)
...
void  inline dmaStoreW(_u8 cr, _u16 data)
{
#ifdef NEOPOP_DEBUG
	if (filter_dma)
...
void inline  dmaStoreL(_u8 cr, _u32 data)
{
#ifdef NEOPOP_DEBUG
	if (filter_dma)
