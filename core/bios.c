...
_u8 bios[0x10000] __attribute__((aligned(64)));		//Holds bios program data
...
void update_rtc(void)
{
	...
	{
		low =cdclock->year - 100;	high = low;		//Years
		ram[0x0091] = ((high / 10) << 4) | (low % 10);

		low = cdclock->month + 1; high = low;			//Months
		ram[0x0092] = ((high / 10) << 4) | (low % 10);

		low = cdclock->day; high = low;				//Days
		ram[0x0093] = ((high / 10) << 4) | (low % 10);

		low = cdclock->hour; high = low;			//Hours
		ram[0x0094] = ((high / 10) << 4) | (low % 10);

		low = cdclock->minute; high = low;			//Minutes
		ram[0x0095] = ((high / 10) << 4) | (low % 10);

		low = cdclock->second % 60; high = low;			//Seconds
		ram[0x0096] = ((high / 10) << 4) | (low % 10);

		ram[0x0097] = ((ram[0x0091] % 4)<<4) | (/*localTime->tm_wday*/ 0x01 & 0x0F); //1st day of week
	}
}
/*
void update_rtc(void)
