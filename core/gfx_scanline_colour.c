...
// this is a hack to speed up things
extern _u16 rgb_lookup[];

//=============================================================================

static inline void Plot(_u8 x, _u16* palette_ptr, _u8 palette, _u8 index, _u8 depth)
{
	//_u16 data16;
	...
	if (negative)
		cfb_scanline[x] = rgb_lookup[(u16)~(palette_ptr[(palette << 2) + index])];
	else
		cfb_scanline[x] = rgb_lookup[(u16)(palette_ptr[(palette << 2) + index])];
}

static inline void drawPattern(_u8 screenx, _u16 tile, _u8 tiley, _u16 mirror, 
...
static inline void gfx_draw_scroll1(_u8 depth)
{
	_u8 tx, row, line;
	_u16 data16;
...
static inline void gfx_draw_scroll2(_u8 depth)
{
	_u8 tx, row, line;
	_u16 data16;
...
void gfx_draw_scanline_colour(void)
{
	...
	//	else data16 = 0;
		...
		{
			_u8 priority, row;
			_u8 sx = ram[0x8800 + (spr << 2) + 2];	//X position
			_u8 sy = ram[0x8800 + (spr << 2) + 3];	//Y position
			_s16 x = sx;
			_s16 y = sy;
			_u16 data16;
			
			data16 = le16toh(*(_u16*)(ram + 0x8800 + (spr << 2)));
