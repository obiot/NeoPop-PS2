...
_u8 dacBufferL[DAC_BUFFERSIZE] __attribute__((aligned(64)));
...
static inline _u16 sample_chip_tone(void)
{
	int i;
	...
	// put it into 16kb scratch pad @ 70001000
    	vol = (int *)(0x70001000); 
...
static inline _u16 sample_chip_noise(void)
{
#if 0
	int i;
#endif
...
void sound_update(_s16* chip_buffer, int length_bytes)
{
	_s16 tmp;
	while (length_bytes > 0)
	{
		...
		tmp=((sample_chip_tone() + sample_chip_noise()) >> 1) -16384;
		if(tmp>=0)tmp++;
		*(chip_buffer++) = tmp;	
		length_bytes--;
	...
	if (filter_sound)
	{
		...
		if (chip == &toneChip)
			...
			system_debug_message("sound (N): Set Noise Mode to %s, Period = %s", nm, pm);
				...
				chip->Period[3] = (n == 3) ? (chip->Period[2] << 1): (UpdateStep << (5+n));
...
void sound_init(int SampleRate)
{
	...
	/* number. */
	UpdateStep = (_u32)(((double)STEP * (SampleRate << 4)) / SOUNDCHIPCLOCK);
