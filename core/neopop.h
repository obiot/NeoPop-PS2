...
typedef struct 
{
	_u8* data __attribute__((aligned(64)));		//Pointer to the rom data
	_u32 length;	//Length of the rom

	_u8 name[16] __attribute__((aligned(64)));	//Null terminated string, holding the Game name

	//For use as flash file and default state name
	_u8 filename[256] __attribute__((aligned(64)));	
}
RomInfo;
...
#define RAM_END		0x00BFFF
	...
	extern _u8 ram[1 + RAM_END - RAM_START]    __attribute__((aligned(64)));
...
#define SCREEN_HEIGHT	152
	...
	extern _u16 *cfb;//[256*256] __attribute__((aligned(64)));
